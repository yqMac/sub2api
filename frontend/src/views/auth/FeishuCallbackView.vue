...
type PendingFeishuCompletion = PendingOAuthExchangeResponse & {
  step?: string
...
async function loadProviderName() {
  try {
    ...
    // Ignore; fallback remains Feishu
  }
}
...
function extractPendingAccountEmail(completion: PendingFeishuCompletion): string {
  return (
...
function resolvePendingAccountAction(
  completion: PendingFeishuCompletion
...
function applyPendingAccountAction(completion: PendingFeishuCompletion) {
...
function applyTotpChallenge(completion: PendingFeishuCompletion): boolean {
...
async function finalizePendingAccountResponse(completion: PendingFeishuCompletion) {
...
async function handleSubmitInvitation() {
  ...
  try {
    ...
    const completion: PendingFeishuCompletion = legacyPendingOAuthToken.value
      ? (
          await apiClient.post<PendingFeishuCompletion>('/auth/oauth/feishu/complete-registration', {
      ...
      : affCode
        ? await completeFeishuOAuthRegistration(invitationCode.value.trim(), decision, affCode)
        : await completeFeishuOAuthRegistration(invitationCode.value.trim(), decision)
...
async function handleContinueLogin() {
  isSubmitting.value = true
  try {
    const completion = await exchangePendingOAuthCompletion(currentAdoptionDecision()) as PendingFeishuCompletion
    await finalizePendingAccountResponse(completion)
  } catch (e: unknown) {
...
async function handleCreateAccount(payload: PendingOAuthCreateAccountPayload) {
  ...
  isSubmitting.value = true
  try {
    const { data } = await apiClient.post<PendingFeishuCompletion>('/auth/oauth/pending/create-account', {
      email: payload.email,
...
async function handleBindLogin() {
  ...
  isSubmitting.value = true
  try {
    const { data } = await apiClient.post<PendingFeishuCompletion>('/auth/oauth/pending/bind-login', {
      email,
      password,
      ...serializeAdoptionDecision(currentAdoptionDecision())
    })
...
onMounted(async () => {
  ...
  try {
    ...
    const completion = await exchangePendingOAuthCompletion() as PendingFeishuCompletion
